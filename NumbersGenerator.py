...
class Mold:
    ...
    def label(Height, Width, Text, X, Y, Font, re, color):
        label = Label(root, width=Width, height=Height, text=Text, font=Font, relief=re, justify=LEFT,
                      background=color, anchor=W)
        label.place(x=X, y=Y)
        return label
...
def function():
    ...
    xLabel, yLabel, n, n1 = 10, 150, 39, 0
    if elements > 200:
        AlertLabel = Mold.label(1, 25, "the max is 200", 250, 70, "arial 12", re="solid", color="white")
    else:
        AlertLabel = Mold.label(1, 25, "", 250, 70, "arial 12", re="flat", color=None)
        for i1 in range(elements):
            x = randrange(int(From), int(to))
...
bx1l = Mold.label(Height=1, Width=15, Text="from", X=10, Y=10, Font="arial 12", re="flat", color="white")
bx1 = Mold.box(Width=10, X=150, Y=10, Font="arial 12")

bx2l = Mold.label(Height=1, Width=15, Text="to", X=10, Y=40, Font="arial 12", re="flat", color="white")
bx2 = Mold.box(Width=10, X=150, Y=40, Font="arial 12")

bx3l = Mold.label(Height=1, Width=15, Text="Nº of elements", X=10, Y=70, Font="arial 12", re="flat", color="white")
